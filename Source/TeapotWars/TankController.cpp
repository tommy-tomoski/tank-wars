// TankController.cpp - Controller class for the player Tanks
//
...
//
// TankController::TankController			- Chapter 19, page 735
//
TankController::TankController(shared_ptr<SceneNode> object)
: m_object(object)
{
...
//
// TankController::VOnLButtonDown				- Chapter 19, page 735
//
bool TankController::VOnPointerButtonDown(const Point &mousePos, const int radius, const std::string &buttonName)
{
	if (buttonName != "PointerLeft")
		return false;
	ActorId actorId = m_object->VGet()->ActorId();
	GCC_ASSERT(actorId != INVALID_ACTOR_ID && _T("The Tank controller isn't attached to a valid actor!"));
...
//
// TankController::OnUpdate				- Chapter 19, page 736
//
void TankController::OnUpdate(DWORD const deltaMilliseconds)  
{
    //
}

bool TankController::VOnKeyDown(const BYTE c)
{
...
bool TankController::VOnKeyUp(const BYTE c)
{
    // update the key table
    m_bKey[c] = false;
