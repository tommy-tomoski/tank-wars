#pragma once
//========================================================================
// TankWarsView.h : source file for the sample game
//
...
class TankWarsHumanView : public HumanView
{
protected:
	bool  m_bShowUI;					// If true, it renders the UI control text
    std::wstring m_gameplayText;

	shared_ptr<TankController> m_pTankController;
	shared_ptr<MovementController> m_pFreeCameraController;
	shared_ptr<SceneNode> m_pTank;
	shared_ptr<StandardHUD> m_StandardHUD; 

public:
	TankWarsHumanView(shared_ptr<IRenderer> renderer); 
	virtual ~TankWarsHumanView();
...
class AITankView : public IGameView 
{
	friend class AITankViewListener;
...
public:
	AITankView(shared_ptr<PathingGraph> pPathingGraph);
	virtual ~AITankView();
