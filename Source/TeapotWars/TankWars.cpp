//========================================================================
// TankWars.cpp : source file for the sample game
//
...
#include "TankWars.h"
#include "TankWarsView.h"
#include "TankWarsNetwork.h"
#include "TankEvents.h"
#include "TankResources.h"


TankWarsApp g_TankWarsApp;
...
//========================================================================
//
// TankWarsApp Implementation     - Chapter 21, page 722
//
//========================================================================
...
//
// TankWarsApp::VCreateGameAndView
//
BaseGameLogic *TankWarsApp::VCreateGameAndView()
{
	m_pGame = GCC_NEW TankWarsLogic();
    m_pGame->Init();
...
HICON TankWarsApp::VGetIcon()
{
	return LoadIcon(GetInstance(), MAKEINTRESOURCE(IDI_ICON1));
}

void TankWarsApp::VRegisterGameEvents(void)
{
...
void TankWarsApp::VCreateNetworkEventForwarder(void)
{
...
void TankWarsApp::VDestroyNetworkEventForwarder(void)
{
...
//========================================================================
//
// TankWarsLogic Implementation       - Chapter 21, page 725
//
//========================================================================

//
// TankWarsLogic::TankWarsLogic
//
TankWarsLogic::TankWarsLogic()
{
...
//
// TankWarsLogic::VChangeState
//
void TankWarsLogic::VChangeState(BaseGameState newState)
{
	...
	switch(newState)
	{
		case BGS_WaitingForPlayers:
		{
			...
			{
				shared_ptr<IGameView> playersView(GCC_NEW TankWarsHumanView(g_pApp->m_Renderer));
			...
			{
				shared_ptr<IGameView> aiView(GCC_NEW AITankView(m_pPathingGraph));
				VAddView(aiView);
			}
			break;
		}
		...
		case BGS_SpawningPlayersActors:
		{
			...
			for (auto it = m_gameViews.begin(); it != m_gameViews.end(); ++it)
			{
				...
				{
					shared_ptr<AITankView> pAiView = static_pointer_cast<AITankView, IGameView>(pView);
...
//
// TankWarsLogic::VAddView
//
void TankWarsLogic::VAddView(shared_ptr<IGameView> pView, ActorId actor)
{
...
//	}
	if (dynamic_pointer_cast<TankWarsHumanView>(pView))
	{
		m_HumanPlayersAttached++;
	}
	else if (dynamic_pointer_cast<AITankView>(pView))
	{
		m_AIPlayersAttached++;
	}
}
...
void TankWarsLogic::VMoveActor(const ActorId id, Mat4x4 const &mat)
{
...
void TankWarsLogic::RequestStartGameDelegate(IEventDataPtr pEventData)
{
	VChangeState(BGS_WaitingForPlayers);
}

void TankWarsLogic::EnvironmentLoadedDelegate(IEventDataPtr pEventData)
{
    ++m_HumanGamesLoaded;
}


// FUTURE WORK - this isn't TankWars specific so it can go into the game agnostic base class
void TankWarsLogic::RemoteClientDelegate(IEventDataPtr pEventData)
{
...
void TankWarsLogic::NetworkPlayerActorAssignmentDelegate(IEventDataPtr pEventData)
{
	if (!m_bProxy)
		return;
	...
	for (auto it = m_gameViews.begin(); it != m_gameViews.end(); ++it)
    {
        ...
        if (pView->VGetType() == GameView_Human)
        {
            shared_ptr<TankWarsHumanView> pHumanView = static_pointer_cast<TankWarsHumanView, IGameView>(pView);
...
void TankWarsLogic::StartThrustDelegate(IEventDataPtr pEventData)
{
...
void TankWarsLogic::EndThrustDelegate(IEventDataPtr pEventData)
{
    shared_ptr<EvtData_StartThrust> pCastEventData = static_pointer_cast<EvtData_StartThrust>(pEventData);
    StrongActorPtr pActor = MakeStrongPtr(VGetActor(pCastEventData->GetActorId()));
    if (pActor)
...
void TankWarsLogic::StartSteerDelegate(IEventDataPtr pEventData)
{
    shared_ptr<EvtData_StartThrust> pCastEventData = static_pointer_cast<EvtData_StartThrust>(pEventData);
    StrongActorPtr pActor = MakeStrongPtr(VGetActor(pCastEventData->GetActorId()));
    if (pActor)
...
void TankWarsLogic::EndSteerDelegate(IEventDataPtr pEventData)
{
    shared_ptr<EvtData_StartThrust> pCastEventData = static_pointer_cast<EvtData_StartThrust>(pEventData);
    StrongActorPtr pActor = MakeStrongPtr(VGetActor(pCastEventData->GetActorId()));
    if (pActor)
...
void TankWarsLogic::TestScriptDelegate(IEventDataPtr pEventData)
{
...
void TankWarsLogic::RegisterAllDelegates(void)
{
	// FUTURE WORK: Lots of these functions are ok to go into the base game logic!
    IEventManager* pGlobalEventManager = IEventManager::Get();
    pGlobalEventManager->VAddListener(MakeDelegate(this, &TankWarsLogic::RemoteClientDelegate), EvtData_Remote_Client::sk_EventType);
    pGlobalEventManager->VAddListener(MakeDelegate(this, &TankWarsLogic::MoveActorDelegate), EvtData_Move_Actor::sk_EventType);
    pGlobalEventManager->VAddListener(MakeDelegate(this, &TankWarsLogic::RequestStartGameDelegate), EvtData_Request_Start_Game::sk_EventType);
    pGlobalEventManager->VAddListener(MakeDelegate(this, &TankWarsLogic::NetworkPlayerActorAssignmentDelegate), EvtData_Network_Player_Actor_Assignment::sk_EventType);
	pGlobalEventManager->VAddListener(MakeDelegate(this, &TankWarsLogic::EnvironmentLoadedDelegate), EvtData_Environment_Loaded::sk_EventType);
	pGlobalEventManager->VAddListener(MakeDelegate(this, &TankWarsLogic::EnvironmentLoadedDelegate), EvtData_Remote_Environment_Loaded::sk_EventType);

	// FUTURE WORK: Only these belong in TankWars.
    pGlobalEventManager->VAddListener(MakeDelegate(this, &TankWarsLogic::StartThrustDelegate), EvtData_StartThrust::sk_EventType);
    pGlobalEventManager->VAddListener(MakeDelegate(this, &TankWarsLogic::EndThrustDelegate), EvtData_EndThrust::sk_EventType);
    pGlobalEventManager->VAddListener(MakeDelegate(this, &TankWarsLogic::StartSteerDelegate), EvtData_StartSteer::sk_EventType);
    pGlobalEventManager->VAddListener(MakeDelegate(this, &TankWarsLogic::EndSteerDelegate), EvtData_EndSteer::sk_EventType);

    pGlobalEventManager->VAddListener(MakeDelegate(this, &TankWarsLogic::TestScriptDelegate), EvtData_ScriptEventTest_FromLua::sk_EventType);
}

void TankWarsLogic::RemoveAllDelegates(void)
{
	// FUTURE WORK: See the note in RegisterDelegates above....
    IEventManager* pGlobalEventManager = IEventManager::Get();
    pGlobalEventManager->VRemoveListener(MakeDelegate(this, &TankWarsLogic::RemoteClientDelegate), EvtData_Remote_Client::sk_EventType);
    pGlobalEventManager->VRemoveListener(MakeDelegate(this, &TankWarsLogic::MoveActorDelegate), EvtData_Move_Actor::sk_EventType);
    pGlobalEventManager->VRemoveListener(MakeDelegate(this, &TankWarsLogic::RequestStartGameDelegate), EvtData_Request_Start_Game::sk_EventType);
    pGlobalEventManager->VRemoveListener(MakeDelegate(this, &TankWarsLogic::NetworkPlayerActorAssignmentDelegate), EvtData_Network_Player_Actor_Assignment::sk_EventType);
	pGlobalEventManager->VRemoveListener(MakeDelegate(this, &TankWarsLogic::EnvironmentLoadedDelegate), EvtData_Environment_Loaded::sk_EventType);
	pGlobalEventManager->VRemoveListener(MakeDelegate(this, &TankWarsLogic::EnvironmentLoadedDelegate), EvtData_Remote_Environment_Loaded::sk_EventType);
	if (m_bProxy)
	{
	    pGlobalEventManager->VRemoveListener(MakeDelegate(this, &TankWarsLogic::RequestNewActorDelegate), EvtData_Request_New_Actor::sk_EventType);
	}

	// FUTURE WORK: These belong in Tank wars!
	pGlobalEventManager->VRemoveListener(MakeDelegate(this, &TankWarsLogic::StartThrustDelegate), EvtData_StartThrust::sk_EventType);
    pGlobalEventManager->VRemoveListener(MakeDelegate(this, &TankWarsLogic::EndThrustDelegate), EvtData_EndThrust::sk_EventType);
    pGlobalEventManager->VRemoveListener(MakeDelegate(this, &TankWarsLogic::StartSteerDelegate), EvtData_StartSteer::sk_EventType);
    pGlobalEventManager->VRemoveListener(MakeDelegate(this, &TankWarsLogic::EndSteerDelegate), EvtData_EndSteer::sk_EventType);

    pGlobalEventManager->VRemoveListener(MakeDelegate(this, &TankWarsLogic::TestScriptDelegate), EvtData_ScriptEventTest_FromLua::sk_EventType);
}

void TankWarsLogic::CreateNetworkEventForwarder(const int socketId)
{
...
void TankWarsLogic::DestroyAllNetworkEventForwarders(void)
{
...
bool TankWarsLogic::VLoadGameDelegate(TiXmlElement* pLevelData)
{
    RegisterTankScriptEvents();
    return true;
}
