...
extern TankWarsApp g_TankWarsApp;
...
MainMenuUI::MainMenuUI()
{
	...
	D3DCOLOR color = D3DXCOLOR((float)128/255, (float)128/255, 0, 1.0f);
	m_SampleUI.SetBackgroundColors(color);

	m_SampleUI.SetFont(0, L"Ariel", height, 0);

	m_SampleUI.AddStatic(0, L"Welcome to tank wars", iX-30, iY, g_SampleUIWidth, height * 2);
	iY += (lineHeight * 1.5);

	m_SampleUI.AddStatic(0, L"Objective: Don't get destroyed!", width / 2 - 30, iY, g_SampleUIWidth, height * 2);
	iY += (lineHeight * 1.5);
...
{
	HRESULT hr;
	DXUT_BeginPerfEvent( DXUT_PERFEVENTCOLOR, L"TankWarsHUD" ); // These events are to help PIX identify what the code is doing
...
//========================================================================
//
// TankWarsHumanView Implementation
//
//========================================================================

//
// TankWarsHumanView::TankWarsHumanView	- Chapter 19, page 724
//
TankWarsHumanView::TankWarsHumanView(shared_ptr<IRenderer> renderer) : HumanView(renderer)
{ 
	m_bShowUI = true; 
...
TankWarsHumanView::~TankWarsHumanView() 
{
    RemoveAllDelegates();
}

//
// TankWarsHumanView::VOnMsgProc				- Chapter 19, page 727
//
LRESULT CALLBACK TankWarsHumanView::VOnMsgProc( AppMsg msg )
{
	if (HumanView::VOnMsgProc(msg))
		return 1;
	...
	if (msg.m_uMsg==WM_KEYDOWN)
	{
		...
		{
			TankWarsLogic *twg = static_cast<TankWarsLogic *>(g_pApp->m_pGame);
			twg->ToggleRenderDiagnostics();

		}
		else if (msg.m_wParam==VK_F9)
		{
			m_KeyboardHandler = m_pTankController;
			m_PointerHandler = m_pTankController;
			m_pCamera->SetTarget(m_pTank);
			m_pTank->SetAlpha(0.8f);
			ReleaseCapture();
			return 1;
		}
...
//
// TankWarsHumanView::VRenderText				- Chapter 19, page 727
//
void TankWarsHumanView::VRenderText()
{
...
//
// TankWarsHumanView::VOnUpdate				- Chapter 19, page 730
//
void TankWarsHumanView::VOnUpdate(unsigned long deltaMs)
{
	HumanView::VOnUpdate( deltaMs );
	...
	if (m_pTankController)
	{
		m_pTankController->OnUpdate(deltaMs);
	}
...
//
// TankWarsHumanView::VOnAttach				- Chapter 19, page 731
//
void TankWarsHumanView::VOnAttach(GameViewId vid, ActorId aid)
{
	HumanView::VOnAttach(vid, aid);
}

bool TankWarsHumanView::VLoadGameDelegate(TiXmlElement* pLevelData)
{
...
void TankWarsHumanView::VSetControlledActor(ActorId actorId) 
{ 
	m_pTank = static_pointer_cast<SceneNode>(m_pScene->FindActor(actorId));
    if (!m_pTank)
    {
        GCC_ERROR("Invalid Tank");
        return;
    }

	HumanView::VSetControlledActor(actorId);

    m_pTankController.reset(GCC_NEW TankController(m_pTank));
    m_KeyboardHandler = m_pTankController;
    m_PointerHandler = m_pTankController;
    m_pCamera->SetTarget(m_pTank);
    m_pTank->SetAlpha(0.8f);
...
void TankWarsHumanView::GameplayUiUpdateDelegate(IEventDataPtr pEventData)
{
...
void TankWarsHumanView::SetControlledActorDelegate(IEventDataPtr pEventData)
{
...
void TankWarsHumanView::DestroyPlayerDelegate(IEventDataPtr pEventData)
{
...
void TankWarsHumanView::RegisterAllDelegates(void)
{
    ...
    pGlobalEventManager->VAddListener(MakeDelegate(this, &TankWarsHumanView::GameplayUiUpdateDelegate), EvtData_Gameplay_UI_Update::sk_EventType);
    pGlobalEventManager->VAddListener(MakeDelegate(this, &TankWarsHumanView::SetControlledActorDelegate), EvtData_Set_Controlled_Actor::sk_EventType);
	//pGlobalEventManager->VAddListener(MakeDelegate(this, &TankWarsHumanView::SetControlledActorDelegate), EvtData_Set_Controlled_Actor::sk_EventType);
	pGlobalEventManager->VAddListener(MakeDelegate(this, &TankWarsHumanView::DestroyPlayerDelegate), EvtData_Destroy_Player::sk_EventType);
}

void TankWarsHumanView::RemoveAllDelegates(void)
{
    IEventManager* pGlobalEventManager = IEventManager::Get();
    pGlobalEventManager->VRemoveListener(MakeDelegate(this, &TankWarsHumanView::GameplayUiUpdateDelegate), EvtData_Gameplay_UI_Update::sk_EventType);
    pGlobalEventManager->VRemoveListener(MakeDelegate(this, &TankWarsHumanView::SetControlledActorDelegate), EvtData_Set_Controlled_Actor::sk_EventType);
	//pGlobalEventManager->VRemoveListener(MakeDelegate(this, &TankWarsHumanView::SetControlledActorDelegate), EvtData_Set_Controlled_Actor::sk_EventType);
	pGlobalEventManager->VRemoveListener(MakeDelegate(this, &TankWarsHumanView::DestroyPlayerDelegate), EvtData_Destroy_Player::sk_EventType);
}
...
///////////////////////////////////////////////////////////////////////////////
//
// AITankView::AITankView					- Chapter 19, page 737
//
AITankView::AITankView(shared_ptr<PathingGraph> pPathingGraph) : IGameView(), m_pPathingGraph(pPathingGraph)
{
    //
}
...
// AITankView::~AITankView					- Chapter 19, page 737
//
AITankView::~AITankView(void)
{
    GCC_LOG("AI", "Destroying AITankView");
}
